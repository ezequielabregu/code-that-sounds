...
sequenceDiagram
    ...
    Output->>Room: Sound travels through room
    Room->>Input: Acoustic reflections captured
    PlayA-->>RecB: Trigger Stop Rec B
    deactivate RecB
    deactivate PlayA
...
```{mermaid}
sequenceDiagram
    participant Input as Microphone
    participant RecA as RecordA.wav
    participant PlayA as PlayA.wav
    participant RecB as RecordB.wav 
    participant PlayB as PlayB.wav
    participant Output as Speaker
    participant Room as Room's Reflections
    
    Note over Input,Room: Generation 1
    Input->>RecA: Record voice
    activate RecA
    RecA->>PlayA: Transfer recording
    deactivate RecA
    activate PlayA
    PlayA->>Output: Play recording
    PlayA->>RecB: Start recording
    deactivate PlayA
    
    Output->>Room: Sound travels
    activate Room
    Room->>Input: Captures reflections
    deactivate Room
    
    Note over Input,Room: Generation 2
    Input->>RecB: Record voice
    activate RecB
    RecB->>PlayB: Transfer recording
    deactivate RecB
    activate PlayB
    PlayB->>Output: Play recording
    PlayB->>RecA: Start recording
    deactivate PlayB
    ...
    Note over Input,Room: Process repeats for multiple generations
