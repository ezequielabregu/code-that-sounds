...
This Pure Data patch demonstrates dynamic additive synthesis using Pure Data's `clone` object to create a scalable system of harmonic oscillators. Unlike fixed multi-oscillator systems, this implementation allows real-time adjustment of the number of active partials, from 1 to 20, while maintaining proper amplitude scaling to prevent clipping. The patch showcases advanced Pure Data programming techniques including dynamic object creation, parameter distribution, and automatic scaling based on psychoacoustic principles.
...
![Fig. ](/assets/screenshots/synthesis/additive/additive-clone.png)
...
|--------|----------|---------------------------|
| `hradio` | Partial count selector | Controls number of active harmonics (1-20) |
...
The additive clone patch orchestrates a sophisticated dynamic synthesis system that adapts the number of active harmonic oscillators in real-time while maintaining proper amplitude relationships and preventing audio clipping. This implementation demonstrates how Pure Data's clone object can be combined with algorithmic parameter generation to create scalable synthesis systems that remain musically coherent across different levels of harmonic complexity.
...
The dynamic additive synthesis system maintains musical coherence through coordinated processing stages:
...
The spectral melody patch implement an algorithmic synthesis process that transforms simple user parameters into complex harmonic spectra through systematic mathematical calculations and dynamic object instantiation. This implementation demonstrates how Pure Data's cloning capabilities can be combined with algorithmic parameter generation to create scalable additive synthesis systems that maintain both mathematical precision and musical expressivity.
...
The signal collection and output coordination stage aggregates the eight independent audio streams into a unified spectral composition through Pure Data's efficient audio signal routing system. Each partial instance employs a `throw~ audio` object that sends its processed audio signal to the global collection point implemented by the `catch~ audio` object in the main patch. This signal routing approach eliminates the need for complex cable connections while maintaining efficient audio summing that preserves the amplitude relationships established during the parameter calculation stage. The collected audio signal represents the complete harmonic spectrum generated by all eight partials, with the spectral content and temporal evolution determined by the interaction between the algorithmic parameter calculations and the stochastic processes operating within each individual partial instance.
...
The spectral melody system maintains mathematical precision and musical coherence through several interconnected processing stages:
...
#### Parameter Distribution Network

| Send/Receive Pair | Function | Scope |
|-------------------|----------|-------|
| `s/r root-freq` | Fundamental frequency | All partial instances |
| `s/r total-time` | Envelope duration base | All partial instances |
| `s/r bang` | Envelope trigger | All partial instances |
